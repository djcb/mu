  ...
  (let* ((perfmsgs (mfind "label:performance"))
    ...
    (for-each (lambda (msg)
		...
		(test-equal "performance" (car (labels msg))))
	      perfmsgs)
    (test-assert (match-label? msg "performance"))
    (test-assert (not (match-label? msg "Performance")))
    (test-assert (match-label? msg (make-regexp "Performance" regexp/icase)))
    (test-assert (match-label? msg "performance\|antlers"))
    (test-assert (not (match-label? msg "mars\|twix"))))
