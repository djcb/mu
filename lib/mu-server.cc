...
Server::Private::make_command_map ()
{
      ...
      cmap.emplace("quit",
                   CommandInfo{{},
                           "quit the mu server",
                           [&](const auto& params){quit_handler(params);}});
...
Server::Private::ping_handler (const Parameters& params)
{
        ...
        proplst.add_prop(":database-path",
                         Sexp::make_string(store().metadata().database_path));
        proplst.add_prop(":root-maildir",
                         Sexp::make_string(store().metadata().root_maildir));
