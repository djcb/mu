...
*** mu
   ...
   - Command-line completion for bash and zsh; see the ~bash-completion~ and
     ~bash-completion-dir~, resp. ~zsh-completion~ and ~zsh-completion-dir~ meson
     build-options if you want to customize this (1.14.3). Add a (hidden)
     =completions= sub-command for generating these, and include *experimental*
     support for the Fish shell (1.14.4)

*** mu4e

**** Work-around

     In Emacs 31.1 (and later) there appears to be a problem with the =substring=
     completion style when completion addresses (see #2946); if you encounter
     this, you can try something like:

   #+begin_src elisp
     (add-hook 'mu4e-compose-mode-hook
         (lambda () (setq-local completion-styles '(basic emacs22))))
   #+end_src

     If installed, the [[https://elpa.gnu.org/packages/orderless.html][orderless]] style can be a good replacement for ~substring~.
     (put before ~basic~ in the example above).
...
**** Updates
   ...
   - For Org-integration, there is a new command ~mu4e-org-agenda-links~ to pop up
     a buffer with a tabulated list of the mu4e links in your ~org-agenda-files~
     (1.14.2).

   - When replying to message with a region marked, cite only the marked region.
     An old feature-request, finally implemented. (1.14.2)

   - In the message-view URLs/email addresses no longer automatically get an
     indicator next to them (=[1]=, =[2]= etc.); these appear only when you invoke
     ~mu4e-view-go-to-url~ (~g~) or ~mu4e-view-save-url~ (~k~). However, you can set
   ...
   - Message "massaging" has been cleaned up an bit, with some non-working
     options removed, and existing ones made to work better (1.14.3)

   - In the main-view, the message-queue options are now hidden when
     ~smtpmail-queue-dir~ is set to ~nil~. (1.14.3)

   - ~mu4e-message-contact-field-matches~ and ~mu4e-message-contact-field-matches~
     now accept a special contact-field =:any= which matches /all/ contact fields
     and =:recip= which matches the recipient contact fields, To:/Cc:/Bcc:.

     Since the names are so long, and frequently useful in user-code (e.g. in
     ~mu4e-refile-folder~ implementations), we also add some ~defalias~ for them:
     ~mu4e-has-address-p~ => ~mu4e-message-contact-field-matches~
     ~mu4e-has-my-address-p~ => ~mu4e-message-contact-field-matches-me~ (1.14.4)
...
*** scm
   ...
   - add convenience procedures ~language?~ for testing a message's language;
     ~label?~ for testing if a message has a given label (1.14.4)

   - add procedure ~recipients~ to get _all_ To/Cc/Bcc recipients of a message
     (1.14.4)

   - rename procedure ~list?~ into ~mailing-list?~, since the former is core binding
     we should not override. This is a breaking change, unfortunately, but
     hopefully should not affect too many users. (1.14.4)
