...
*** mu4e
   ...
   - ~mu4e-compose-resend~ now honors ~mu4e-sent-messages-behavior~ (1.14.4)
...
*** scm
   ...
   - add convenience procedures ~language?~ for testing a message's language;
     ~match-label?~ for testing if a message has a given label, ~tags~ for getting
     the message tags (1.14.4)

   - Add procedures to get contact list for messages: ~contacts~ (From/To/Cc/Bcc)
     and ~recipients~ (To/Cc/Bcc). Add ~match-contact?~, ~imatch-contact?~ to match
     (lists of) contacts against regular expressions (1.14.4)
