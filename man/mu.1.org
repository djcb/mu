...
- *cfind*: find contacts
- *completions*: generate command-line completions (hidden)
...
The *completions* sub-command is hidden from the normal output of *mu* and does not
have its own manpage.

#+begin_example
$ mu completions bash|fish|zsh
#+end_example>

You can use it to generate command-line completion files. However, note that *mu*
already does this for you upon installation.
